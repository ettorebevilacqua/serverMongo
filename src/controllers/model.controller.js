...
const { ModelService, addFieldUser } = require('../services');

function modelController(modelService, options = {}) {

  const create = catchAsync(async (req, res) => {
    const item = await modelService.create(req.body, req.user, options.onCreate && options.onCreate);
...
module.exports = (Model, options) => {
  ...
  return modelController(service, options);
}