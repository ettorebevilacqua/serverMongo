...
const moduliCtrl = require('../../controllers/moduli.controller');
const { Modulo, QuestionModuli } = require('../../models');

const modelCtrl = modelController(Modulo);
const modelQuestionCtrl = modelController(QuestionModuli);
...
router
    .route('/')
    .post(auth('admin'), validate(modelValidation.create), modelCtrl.create)
...
router
    ...
    .patch(auth('admin',), validate(modelValidation.update), modelCtrl.update)
    .delete(auth('admin'), validate(modelValidation.delete), modelCtrl.delete);


router
    .route('/question/:id')
    .get(auth('admin'), validate(modelValidation.getItem), modelQuestionCtrl.getItem)
    .patch(auth('admin',), validate(modelValidation.update), modelQuestionCtrl.update)
    .delete(auth('admin'), validate(modelValidation.delete), modelQuestionCtrl.delete);

router
    .route('/token/:token')
    .get(validate(moduliValidation.getItemToken), moduliCtrl.getModuloGuest);
 ...
 * /moduli/question:
 *   post:
 *     summary: Create a question Moduli
 *     description: Only admins can create other Moduli.
 *     tags: [questionModuli]
 *     security:
 *       - bearerAuth: []
